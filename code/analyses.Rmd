...
data_requests <- 
  read_excel("../data/requests for data/data requests.xlsx", sheet = "contacted") |>
  janitor::clean_names() |>
  mutate(data_sharing_statement = ifelse(data_sharing_statement == "NA", NA, data_sharing_statement),
         ...
         data_sharing_statement_type = ifelse(data_sharing_statement_type == "NA", NA, data_sharing_statement_type))
...
counts <- data_requests|>
  drop_na(data_sharing_statement_type, data_sharing_statement_adhered_to) |>
  mutate(data_sharing_statement_adhered_to = as.logical(data_sharing_statement_adhered_to)) |>
  group_by(data_sharing_statement_type) |>
  summarize(n_sharing_statement_type = n(),
            n_sharing_statement_adhered_to = sum(data_sharing_statement_adhered_to, na.rm = TRUE),
            percent_shared_data = round_half_up(mean(data_sharing_statement_adhered_to, na.rm = TRUE)*100, 1)) 
...
Does availability statements predict sharing

```{r}

counts2 <- data_requests |>
  mutate(data_sharing_statement_binary = ifelse(data_sharing_statement == FALSE, FALSE, TRUE)) |>
  drop_na(data_sharing_statement_binary, shared_data) |>
  group_by(data_sharing_statement_binary) |>
  summarize(n_data_sharing_statement = n(),
...
tab_results <- counts2 |>
  select(n_data_sharing_statement, n_shared_data) |>
  as.matrix()
